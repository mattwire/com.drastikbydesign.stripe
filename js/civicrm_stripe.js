...
(function($, CRM) {
  ...
  function loadStripeBillingBlock() {
    ...
    if ($stripePubKey.length) {
      if (!$().Stripe) {
        $.getScript('https://js.stripe.com/v2/', function () {
          Stripe.setPublishableKey($('[name=stripe_pub_key]').val());
        });
      }
    }
    ...
    function submit(event) {
      ...
      if (isWebform) {
        ...
        stripeProcessorId = $('[name=stripe_id]').val();
      ...
      if (!$('[name=stripe_pub_key]').length || !($('[name=stripe_pub_key]').val())) {
  ...
  function getBillingForm() {
    var $billingForm = $('[name=stripe_pub_key]').closest('form');
